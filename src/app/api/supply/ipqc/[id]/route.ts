...
async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!await verifyPermission(user.role, 'ipqc.update', user.id)) {
    return NextResponse.json({ error: '权限不足' }, { status: 403 })
  }

  const { id } = await params
  const old = await (prisma as any).iPQC.findUnique({ where: { id } })
  if (!old) return NextResponse.json({ error: '记录不存在' }, { status: 404 })

  let body: any
  try { body = await req.json() } catch { body = {} }

  const record = await (prisma as any).iPQC.update({ where: { id }, data: body, include: INCLUDE })
  ...
  if (old.result !== 'FAIL' && body.result === 'FAIL') {
    try {
      ...
      console.error('[NCR_AUTO] IPQC 自动处置记录失败:', e)
    }
  }

  return NextResponse.json({ iPQC: record })
}
